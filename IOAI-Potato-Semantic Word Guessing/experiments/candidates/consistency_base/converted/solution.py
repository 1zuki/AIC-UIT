...
class PublicEmbeddingPlayer:
    ...
    def __init__(self):
        self.adaptive = np.full(count, 1.0 / count, dtype=np.float64)
        self.robust = self.priors[:, None] * np.full(
    ...
    def normalize_available(self):
        ...
        adaptive_total = float(self.adaptive.sum())
        if not np.isfinite(adaptive_total) or adaptive_total <= 1e-300:
            self.adaptive.fill(0.0)
            self.adaptive[available] = 1.0 / available_count
        else:
            self.adaptive /= adaptive_total
        for index in range(3):
            total = float(self.robust[index].sum())
            if not np.isfinite(total) or total <= 1e-300:
                self.robust[index].fill(0.0)
                self.robust[index, available] = (
                    self.priors[index] / available_count
                )
            else:
                self.robust[index] *= self.priors[index] / total

    def observe(self, message):
        if self.last_proposal >= 0:
            self.used[self.last_proposal] = True
            self.adaptive[self.last_proposal] = 0.0
            self.robust[:, self.last_proposal] = 0.0
        ...
        self.adaptive *= likelihood(margin, 0)
        for index in range(3):
            self.robust[index] *= likelihood(margin, index)
        self.adaptive[self.used] = 0.0
        self.robust[:, self.used] = 0.0
        self.normalize_available()
        available = ~self.used
    ...
    def choose(self, champion, turn):
        ...
        if self.switched:
            posterior = self.robust.sum(axis=0)
        else:
            posterior = self.adaptive
        if turn >= 30:
